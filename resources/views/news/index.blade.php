...
<head>
    ...
    <title>GameDock News - Latest Gaming Updates</title>
...
<body class="bg-gamedock-dark text-white font-sans antialiased overflow-x-hidden selection:bg-gamedock-blue selection:text-white">
    ...
    <div class="bg-gamedock-darker/90 py-3 border-b border-white/5 flex justify-center overflow-x-auto">
        <ul class="flex gap-4 sm:gap-8 text-xs md:text-sm font-semibold text-gray-400 tracking-wide px-4 whitespace-nowrap">
            ...
            <li><a href="{{ route('news.index') }}" class="text-white hover:drop-shadow-[0_0_5px_rgba(255,255,255,0.8)] transition">News</a></li>
        </ul>
    </div>

    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-12">
        <div class="mb-10">
            <h1 class="text-3xl sm:text-4xl font-black tracking-widest uppercase text-white drop-shadow-md">Latest News</h1>
            <div class="h-1 w-32 bg-gamedock-blue mt-3 rounded-full shadow-[0_0_10px_rgba(75,118,196,0.8)]"></div>
            <p class="mt-4 text-gray-400 text-lg">Stay updated with the latest gaming announcements, sales, and platform news.</p>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            @foreach($news as $item)
            <div class="bg-gamedock-card rounded-xl overflow-hidden shadow-xl border border-white/5 hover:border-gamedock-blue/50 transition-all duration-300 group">
                <div class="h-48 {{ $item['image'] ? 'bg-cover bg-center' : 'bg-gradient-to-br from-gamedock-blue/20 to-gamedock-dark' }}"
                        @if($item['image']) style="background-image: url('{{ asset($item['image']) }}')" @endif>
                </div>
                <div class="p-6">
                    <span class="text-xs font-bold text-gamedock-blue uppercase tracking-wider">{{ \Carbon\Carbon::parse($item['date'])->format('M d, Y') }}</span>
                    <h2 class="text-2xl font-bold mt-2 mb-3 group-hover:text-gamedock-blue transition-colors">{{ $item['title'] }}</h2>
                    <p class="text-gray-400 leading-relaxed">{{ $item['excerpt'] }}</p>
                </div>
            </div>
            @endforeach
        </div>
